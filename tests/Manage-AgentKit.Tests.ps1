...
$fakeAgy = Join-Path $repoRoot 'tests\fixtures\fake-agy.ps1'
...
function Invoke-Manager {
    param([string[]]$Arguments, [string]$FixtureHome = $homeRoot, [string]$AgyCommand = $fakeAgy)
    $base = @('-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', $manager, '-RepositoryRoot', $repoRoot, '-HomeRoot', $FixtureHome, '-OutputFormat', 'Json', '-AllowDirtyArtifact', '-AntigravityCommand', $AgyCommand)
...
function Get-FakeAgyRegistry {
    param([string]$FixtureHome = $homeRoot)
    $path = Join-Path $FixtureHome '.fake-agy-registry.json'
...
try {
    ...
    Assert-Equal 1 @($check1.Data.targets | Where-Object deployment -eq 'AntigravityManaged').Count 'Antigravity shared plugin uses official managed installation'
    Assert-True ([string]$check1.Data.antigravityCommandDigest -match '^[a-f0-9]{64}$') 'Check returns a bounded Antigravity command approval digest'

    $identityHome = Join-Path $fixtureRoot 'command-identity-home'
    $mutableFakeAgy = Join-Path $fixtureRoot 'mutable-fake-agy.ps1'
    $identityMarker = Join-Path $fixtureRoot 'changed-command-executed.txt'
    [IO.File]::Copy($fakeAgy, $mutableFakeAgy, $false)
    $identityCheck = Invoke-Manager -FixtureHome $identityHome -AgyCommand $mutableFakeAgy -Arguments @('-Mode', 'Check', '-Release', $release1, '-ArtifactRoot', $artifact1, '-Targets', 'Antigravity')
    Assert-Equal 2 $identityCheck.ExitCode 'command identity fixture is installable before replacement'
    $markerLiteral = $identityMarker.Replace("'", "''")
    [IO.File]::WriteAllText($mutableFakeAgy, "[IO.File]::WriteAllText('$markerLiteral', 'executed'); throw 'changed fake agy executed'", (New-Object Text.UTF8Encoding($false)))
    $identityChanged = Invoke-Manager -FixtureHome $identityHome -AgyCommand $mutableFakeAgy -Arguments @('-Mode', 'Install', '-Release', $release1, '-ArtifactRoot', $artifact1, '-Targets', 'Antigravity', '-PlanDigest', [string]$identityCheck.Data.planDigest, '-AntigravityCommandDigest', [string]$identityCheck.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite')
    Assert-Equal 1 $identityChanged.ExitCode 'changed Antigravity command is rejected before mutation planning'
    Assert-True (@($identityChanged.Data.errors | Where-Object { $_ -match 'command digest changed' }).Count -eq 1) 'changed Antigravity command rejection reason'
    Assert-True (-not [IO.File]::Exists($identityMarker)) 'changed Antigravity command is never executed'
    Assert-True (-not [IO.Directory]::Exists($identityHome)) 'changed Antigravity command cannot create the bounded HomeRoot'
    ...
    $wrongDigest = Invoke-Manager -Arguments @('-Mode', 'Install', '-Release', $release1, '-ArtifactRoot', $artifact1, '-Targets', 'All', '-PlanDigest', ('0' * 64), '-AntigravityCommandDigest', [string]$check1.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite')
    ...
    $install1 = Invoke-Manager -Arguments @('-Mode', 'Install', '-Release', $release1, '-ArtifactRoot', $artifact1, '-Targets', 'All', '-PlanDigest', [string]$check1.Data.planDigest, '-AntigravityCommandDigest', [string]$check1.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite')
    ...
    foreach ($relativePath in @('.agents\plugins\yohan-agent-kit', 'plugins\yohan-agent-kit', '.cursor\plugins\local\yohan-agent-kit', '.gemini\antigravity-cli\plugins\yohan-agent-kit', '.yohan-agent-kit\active')) {
    ...
    $antigravityEntry = Get-Item -LiteralPath (Join-Path $homeRoot '.gemini\config\plugins\yohan-agent-kit') -Force
    Assert-True ($antigravityEntry.PSIsContainer -and ($antigravityEntry.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0) 'Antigravity official install is an ordinary directory'
    Assert-Equal 'yohan-agent-kit' ([string]::Join(',', @(Get-FakeAgyRegistry))) 'Antigravity official registry records the plugin'
    ...
    $activeHardLinkOutsideHash = Get-TestSha256File -Path $activeHardLinkOutside
    ...
    Assert-Equal $activeHardLinkOutsideHash (Get-TestSha256File -Path $activeHardLinkOutside) 'active state hard link cannot modify outside bytes'
    [IO.File]::Delete($activeStatePath)
    [IO.File]::WriteAllBytes($activeStatePath, $activeStateBytes)
    $noChange = Invoke-Manager -Arguments @('-Mode', 'Install', '-Release', $release1, '-ArtifactRoot', $artifact1, '-Targets', 'All', '-PlanDigest', [string]$healthy1.Data.planDigest, '-AntigravityCommandDigest', [string]$healthy1.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite')
    ...
    $wrongMode = Invoke-Manager -Arguments @('-Mode', 'Install', '-Release', $release2, '-ArtifactRoot', $artifact2, '-Targets', 'All', '-PlanDigest', [string]$check2.Data.planDigest, '-AntigravityCommandDigest', [string]$check2.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite')
    ...
    $update2 = Invoke-Manager -Arguments @('-Mode', 'Update', '-Release', $release2, '-ArtifactRoot', $artifact2, '-Targets', 'All', '-PlanDigest', [string]$check2.Data.planDigest, '-AntigravityCommandDigest', [string]$check2.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite')
    ...
    $restore1 = Invoke-Manager -Arguments @('-Mode', 'Restore', '-BackupId', [string]$install1.Data.backupId, '-PlanDigest', [string]$restore1Check.Data.planDigest, '-AntigravityCommandDigest', [string]$restore1Check.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite')
    ...
    Assert-Equal 0 @((Get-FakeAgyRegistry)).Count 'initial restore removes Antigravity registry entry'

    $managedFaultCheck = Invoke-Manager -Arguments @('-Mode', 'Check', '-Release', $release3, '-ArtifactRoot', $artifact3, '-Targets', 'All')
    $managedFault = Invoke-Manager -Arguments @('-Mode', 'Install', '-Release', $release3, '-ArtifactRoot', $artifact3, '-Targets', 'All', '-PlanDigest', [string]$managedFaultCheck.Data.planDigest, '-AntigravityCommandDigest', [string]$managedFaultCheck.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite', '-TestFault', 'AfterAntigravityInstall')
    Assert-Equal 1 $managedFault.ExitCode 'failure after Antigravity official install reports error'
    Assert-True (@($managedFault.Data.errors | Where-Object { $_ -match 'mutation rolled back' }).Count -eq 1) 'Antigravity failure reports rollback'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $homeRoot '.gemini\config\plugins\yohan-agent-kit'))) 'Antigravity rollback removes managed directory'
    Assert-Equal 0 @((Get-FakeAgyRegistry)).Count 'Antigravity rollback removes registry entry'

    $check3 = Invoke-Manager -Arguments @('-Mode', 'Check', '-Release', $release3, '-ArtifactRoot', $artifact3, '-Targets', 'All')
    $fault = Invoke-Manager -Arguments @('-Mode', 'Install', '-Release', $release3, '-ArtifactRoot', $artifact3, '-Targets', 'All', '-PlanDigest', [string]$check3.Data.planDigest, '-AntigravityCommandDigest', [string]$check3.Data.antigravityCommandDigest, '-ApproveGlobalHomeWrite', '-TestFault', 'AfterFirstTarget')
    ...
    $antigravityEscapeHome = Join-Path $fixtureRoot 'antigravity-escape-home'
    $antigravityOutside = Join-Path $fixtureRoot 'antigravity-outside'
    $null = New-Item -ItemType Directory -Path $antigravityEscapeHome -Force
    $null = New-Item -ItemType Directory -Path $antigravityOutside -Force
    $outsideMarker = Join-Path $antigravityOutside 'preserve.txt'
    [IO.File]::WriteAllText($outsideMarker, 'preserve me')
    $null = New-Item -ItemType Junction -Path (Join-Path $antigravityEscapeHome '.gemini') -Target $antigravityOutside
    $antigravityEscape = Invoke-Manager -FixtureHome $antigravityEscapeHome -Arguments @('-Mode', 'Check', '-Release', $release1, '-ArtifactRoot', $artifact1, '-Targets', 'Antigravity')
    Assert-Equal 1 $antigravityEscape.ExitCode 'Antigravity destination below a parent junction fails closed'
    Assert-True (@($antigravityEscape.Data.errors | Where-Object { $_ -match 'reparse point' }).Count -eq 1) 'Antigravity parent-junction rejection reason'
    Assert-Equal 'preserve me' ([IO.File]::ReadAllText($outsideMarker)) 'Antigravity parent junction cannot change outside bytes'
