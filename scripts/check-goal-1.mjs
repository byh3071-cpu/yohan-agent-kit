...
// ─── goal 1 고유 검증 (직접 추가) ───────────────────────────────
const read = (p) => existsSync(p) ? readFileSync(p, 'utf-8').replace(/^\uFEFF/, '') : null
const adrSkill = read('skills/adr-cycle/SKILL.md')
const goalSkill = read('skills/goal-cycle/SKILL.md')
const adrUi = read('skills/adr-cycle/agents/openai.yaml')
const goalUi = read('skills/goal-cycle/agents/openai.yaml')

must(adrSkill?.startsWith('---\nname: adr-cycle\ndescription:'), 'adr-cycle frontmatter')
must(goalSkill?.startsWith('---\nname: goal-cycle\ndescription:'), 'goal-cycle frontmatter')
must(adrSkill?.includes('references/review-checklist.md'), 'adr-cycle reference 연결')
must(goalSkill?.includes('(reference.md)'), 'goal-cycle reference 연결')
must(adrUi?.includes('$adr-cycle') && adrUi?.includes('allow_implicit_invocation: true'), 'adr-cycle Codex UI metadata')
